...
namespace Exercicio4
{
    ...
    class TrabalhadorESupervisor
    {
        ...
        private void LiberarTurno()
        {
            lock (trava)
            {
                turnoLiberado = true; // libera os trabalhadores
        ...
        private void AguardarTurno(int id)
        {
            lock (trava)
            {
                while (!turnoLiberado)
                {
                    Monitor.Wait(trava); // aguarda sinal
                }
            }
        }
        ...
        private void RealizarTrabalho(int id)
        {
            lock (trava)
            {
                ...
                Thread.Sleep(new Random().Next(500, 1000)); // tempo de trabalho
                Console.WriteLine($"Trabalhador {id}: Finalizou o trabalho.");
                turnoLiberado = false; //  turno foi usado
            }
        }
    }
